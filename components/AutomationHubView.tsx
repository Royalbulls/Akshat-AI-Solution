...
const AutomationHubView: React.FC<AutomationHubViewProps> = () => {
    ...
    const handleGenerateProject = async () => {
        ...
        try {
            const project = await generateStructuredProject(selectedTemplate.title, userInput, selectedLanguage);
    ...
    const handleRunAgent = async () => {
        ...
        try {
            ...
            setAgentLog(prev => [...prev, "Step 2: Perception & Action (Researching & Drafting)..."]);
            ...
            setAgentLog(prev => [...prev, ">> Accessing Real-time Knowledge via Google Search..."]);
            await new Promise(r => setTimeout(r, 1500));
            updatePlanStatus('Perception', 'completed');
            updatePlanStatus('Action', 'in-progress');
            setAgentLog(prev => [...prev, ">> Synthesizing data into comprehensive draft..."]);
            ...
            setAgentLog(prev => [...prev, ">> Verifying Facts and Data Points..."]);
    ...
    const handleExportBook = () => {
        ...
        const htmlContent = `
            ...
            <head>
                ...
                <style>
                    ...
                    h1 { text-align: center; margin-bottom: 40px; font-size: 3em; }
                    h2 { margin-top: 40px; border-bottom: 2px solid #eee; padding-bottom: 10px; }
                    img { max-width: 100%; height: auto; margin: 20px 0; border-radius: 8px; display: block; margin-left: auto; margin-right: auto; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
                    .section { margin-bottom: 50px; page-break-after: always; }
                    .meta { text-align: center; color: #666; font-size: 0.9em; margin-top: -20px; margin-bottom: 50px; }
                    @media print { .section { page-break-after: always; } }
                </style>
            </head>
            <body>
                <h1>${activeProject.title}</h1>
                <p class="meta">Generated by Akshat AI Automation Studio</p>
        ...
        a.href = url;
        a.download = `${activeProject.title.replace(/\s+/g, '_')}_Book_Export.html`;
        a.click();
    };
    ...
    return (
        <div className="h-full bg-gray-900 text-white p-4 md:p-8 overflow-y-auto animate-fade-in-slide-up flex flex-col items-center w-full">
            <header className="text-center mb-10 max-w-2xl w-full">
                ...
                <h1 className="text-4xl font-bold neon-text-gold mb-3">Enterprise Automation Studio</h1>
                <p className="text-gray-400 text-lg">Transform raw ideas into structured outputs like Books, Research Papers, and Comic Scripts using complex reasoning chains.</p>
            </header>
                        ...
                        {selectedTemplate.id !== 'autonomous_agent' && (
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full bg-gray-900 border border-gray-600 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all"
                                >
